...
class SpaCard extends LitElement {
  ...
  _renderTab() {
    ...
    if (this._tab === 'cam') {
      return html`<div class="cam-view">${c.entity_camera ? html`<hui-image .hass=${this.hass} .cameraImage=${c.entity_camera} cameraView="live"></hui-image>` : 'Caméra non configurée'}</div>`;
    }
  ...
  static styles = css`
    ...
    .m-val { display: block; font-size: 18px; font-weight: 200; }
    .m-label { font-size: 8px; opacity: 0.5; }

    .main-gauge-area { display: flex; align-items: center; gap: 15px; }
    .temp-btn { width: 45px; height: 45px; border-radius: 50%; background: var(--glass); border: 1px solid rgba(255,255,255,0.1); display: flex; align-items: center; justify-content: center; font-size: 20px; cursor: pointer; transition: 0.2s; user-select: none; }
    .temp-btn:active { background: var(--accent); color: #000; transform: scale(0.9); }
    ...
    .water-val { font-size: 40px; font-weight: 100; color: var(--accent); text-shadow: 0 0 15px rgba(0,249,249,0.5); }
    ...
    .g-label { font-size: 10px; opacity: 0.5; margin-bottom: 5px; }
    .g-val { font-size: 18px; color: var(--accent); }

    .sw-grid { display: grid; grid-template-columns: repeat(3, 1fr); gap: 10px; width: 100%; }
    .sw-btn { background: var(--glass); padding: 10px; border-radius: 12px; display: flex; flex-direction: column; align-items: center; gap: 5px; border: 1px solid rgba(255,255,255,0.1); cursor: pointer; transition: 0.2s; }
    .sw-btn ha-icon { --mdc-icon-size: 20px; opacity: 0.5; }
    .sw-btn span { font-size: 9px; }
    .sw-btn.on { border-color: var(--accent); background: rgba(0,249,249,0.1); }
    .sw-btn.on ha-icon { color: var(--accent); opacity: 1; }

    .cam-view { width: 100%; border-radius: 15px; overflow: hidden; background: #000; }
