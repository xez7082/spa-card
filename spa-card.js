...
class SpaCard extends LitElement {
  ...
  static getStubConfig() {
    return { card_title: 'MY SPA', blur_amount: 15, card_height: '580px' };
  }
  ...
  _changeTemp(offset) {
    ...
    const mn = this.config.target_temp_min ?? this.hass.states[id].attributes?.min ?? 10;
    const mx = this.config.target_temp_max ?? this.hass.states[id].attributes?.max ?? 45;
