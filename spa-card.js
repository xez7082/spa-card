// SPA Card V5 – Ultra Design, Drag & Drop, Fully Reactive, HACS Ready
...
class SpaCard extends LitElement {
  ...
  _getState(id) {
    if(!this.hass || !id || !this.hass.states[id]) return { val:'?', unit:'', active:false, icon:'mdi:help-circle' };
    ...
    return html`
      <ha-card class="spa-card" style="background-image:url('${c.background_image||''}');">
        ...
        ${c.camera_entity ? html`
          <div class="gb cam draggable" data-key="camera">
  ...
  static styles = css`
    ...
    .sw:hover {
      box-shadow:0 0 14px cyan,0 0 8px rgba(255,255,255,0.25);
    ...
    @media(max-width:600px){
      .btns-g { grid-template-columns:repeat(2,1fr); }
...
console.info('SPA-CARD V5 Ultra Design loaded – fully reactive, drag & drop + HACS ready');
